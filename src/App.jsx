...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubP = onSnapshot(
      profileRef,
      snap => {
        if (snap.exists()) setAccountNumber(snap.data().accountNumber)
      },
      err => console.error(err)
    )
  ...
  const authActions = {
    signUp: async (email, password) => {
      try {
        ...
        setNotif({ type: 'success', message: 'Account created.' })
        return { ok: true }
      } catch (e) {
        console.error(e)
    ...
    signIn: async (email, password) => {
      try {
        ...
        setNotif({ type: 'info', message: 'Signed in.' })
        return { ok: true }
      } catch (e) {
        console.error(e)
        return { ok: false, error: e.message }
    ...
    reset: async email => {
      try {
        ...
        setNotif({ type: 'info', message: 'Reset email sent.' })
      } catch (e) {
        console.error(e)
        setNotif({ type: 'error', message: 'Reset failed.' })
      }
    },
    ...
    signOut: async () => {
      try {
        ...
        setNotif({ type: 'info', message: 'Signed out.' })
      } catch (e) {
        console.error(e)
      }
    }
  ...
  const handleTransfer = async ({
    ...
    if (!user) {
      setNotif({ type: 'alert', message: 'Sign in required.' })
      return
    }

    const num = parseFloat(amount)
    if (isNaN(num) || num <= 0) {
      setNotif({ type: 'error', message: 'Invalid amount.' })
      return
    }
    ...
    if (num > liveBalance) {
      setNotif({
        type: 'alert',
        message: `Insufficient funds (₦${liveBalance.toLocaleString()})`
      })
      return
    }
    ...
    setNotif({
      type: 'info',
      message: 'Processing transfer and categorization...'
    })

    try {
      const payload = {
        prompt: `Categorize this transaction into ['P2P Transfer','Bank Transfer','Bills','Groceries','Food & Dining','Transport','Utilities','Shopping','Other']: Transfer of ₦${num} to ${recipientName} (${recipientAccount})`
      }
      ...
      const balanceRef = doc(
        db,
        `artifacts/${APP_ID}/users/${user.uid}/user_data/balance`
      )

      await updateDoc(balanceRef, {
        balance: Math.max(0, liveBalance - num),
        lastUpdated: Timestamp.now()
      })

      const txCol = collection(
        db,
        `artifacts/${APP_ID}/public/data/transactions`
      )
      ...
      setNotif({
        type: 'success',
        message: `Sent ₦${num.toLocaleString()} — ${category}`
      })
    } catch (e) {
      console.error(e)
      setNotif({
        type: 'security',
        message: 'Transfer failed. Funds secure.'
      })
    } finally {
    ...
    <div className="container">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="btn btn-primary" onClick={authActions.signOut}>
            Sign Out
          </button>
        </div>
      </header>
      ...
      <div className="mt-6">
        <button className="btn" onClick={() => setView(v => (v === 'home' ? 'analysis' : 'home'))}>
          Toggle View
        </button>
      </div>

      <Notification notification={notif} setNotif={setNotif} />
    </div>
  )
}
